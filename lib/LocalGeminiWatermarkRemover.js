...
(function () {
  ...
  const FLOW_ANCHORS = {
    ...
    "1080x1920": 89,   // -> (943, 1783)
    "1920x1080": 89,   // -> (1783, 943)
  ...
  function locate(imageData, mask) {
  ...
  // ── V7 Step 1: Exact Alpha Unblend (Gain = 1.00) ─────────────────────────────
  function applyUnblend(imageData, mask, pos) {
    ...
    for (let j = 0; j < mh; j++) {
      ...
      for (let i = 0; i < mw; i++) {
        const al = Math.min(UNBLEND_CLIP, a[j * mw + i]); // Gain = 1.00 (no multiplier)
