...
(function () {
  ...
  async function process(blob, options = {}) {
    ...
    try {
      ...
      onProgress({ phase: "processing", frame: 0, total: 96, percent: 0 });
      ...
      for await (const s of runSink.samples()) {
        ...
        // Video watermarks are digitally burned at deterministic bottom-right offsets.
        // We use the exact mathematically calculated anchor. No NCC search is needed
        // because it would just introduce +/- 2px drift from interpolation/noise.
        ...
        // Step 3: Edge-aware de-ringing (eliminates emboss effect from video compression)
        ...
        frameCount++;
        if (frameCount % 4 === 0) {
          const percent = Math.min(95, Math.round((frameCount / 96) * 100));
          onProgress({ phase: "processing", frame: frameCount, total: 96, percent });
        }
      }
      ...
      // 7. Mux Audio packets
      if (aSource && aPackets) {
        onProgress({ phase: "audio", percent: 96 });
        let first = true;
      ...
      onProgress({ phase: "completed", frame: frameCount, total: frameCount, percent: 100 });
